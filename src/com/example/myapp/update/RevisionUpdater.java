...
import com.example.myapp.orm.Revision;
...
public class RevisionUpdater extends Updater{
	...
	private List<Revision> getRevisionListFromServer() {
		...
		String jsonBody = a.asyncGet(ServerContract.getInstance(mContext).getCollections());
		Log.d(TAG, ServerContract.getInstance(mContext).getCollections());
	...
	private Revision findRevisionByName(Revision revToFind, List<Revision> revListWhereFind) {
		for (Revision r : revListWhereFind){
			if(revToFind.name.equals(r.name)) return r;
 		}
